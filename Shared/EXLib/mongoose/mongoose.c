...
#define LOG_DEBUG(fmt, ...)									\
	do {													\
		if (g_log_handler != NULL) {						\
			g_log_handler(LL_DEBUG, __FILE__, __LINE__, fmt, ## __VA_ARGS__);	\
		}													\
	} while (0)
...
#define LOG_ERROR(fmt, ...)									\
	do {													\
		if (g_log_handler != NULL) {						\
			g_log_handler(LL_ERROR, __FILE__, __LINE__, fmt, ## __VA_ARGS__);	\
		}													\
	} while (0)
...
#define LOG_WARN(fmt, ...)									\
	do {													\
		if (g_log_handler != NULL) {						\
			g_log_handler(LL_WARN, __FILE__, __LINE__, fmt, ## __VA_ARGS__);	\
		}													\
	} while (0)
...
#define LOG_INFO(fmt, ...)									\
	do {													\
		if (g_log_handler != NULL) {						\
			g_log_handler(LL_INFO, __FILE__, __LINE__, fmt, ## __VA_ARGS__);	\
		}													\
	} while (0)
...
//#define LOG(l, x)                                     \
//  do {                                                \
//	if(g_log_handler != NULL && l <= g_log_level ) { \
//		g_log_handler ("mongoose %s, %d:", __FILE__, __LINE__);	  \
//		g_log_handler x; 								 \
//	}												   \
//    else if (cs_log_print_prefix(l, __FILE__, __LINE__)) { \
//      cs_log_printf x;                                \
//    }                                                 \
//  } while (0)
...
int cs_log_print_prefix(enum cs_log_level level, const char *file, int ln) {
  ...
  *(--q) = ':';
  ...
  if (cs_log_file == NULL) cs_log_file = fopen("moo.log", "a");
...
mg_log_handler g_log_handler = NULL;
int g_log_level = 0;
void mg_set_log_handler(mg_log_handler handler)
{
	g_log_handler = handler;
}


void mg_set_log_level(int level)
{
	g_log_level = level;
}
...
#ifdef __unix__
#include <sys/epoll.h>
void mg_remove_from_epoll(struct mg_connection* nc) {
	if (nc->sock == INVALID_SOCKET || nc->epoll_events == 0) {
		LOG(LL_ERROR, ("invalid sock or evnets, nc: %p, sock:%d, events:%d", nc, nc->sock, nc->epoll_events));
		return;
	}
	nc->epoll_events = 0;
	struct epoll_event ev;
	ev.events = nc->epoll_events;
	if (epoll_ctl(nc->mgr->epoll_fd, EPOLL_CTL_DEL, nc->sock, &ev) < 0) {
		LOG(LL_ERROR, ("epoll_ctl del, error, %p", nc));
	}
	else {
		DBG(("epoll_ctl del succ, nc:%p, fd:%d", nc, nc->sock));
	}

}
void mg_add_to_epoll(struct mg_connection* nc, int op) {
	if (nc->sock == INVALID_SOCKET)	{
		LOG(LL_ERROR, ("invalid sock  nc: %p, sock:%d", nc, nc->sock));
		return;
	}
	struct epoll_event ev;
	ev.events = nc->epoll_events;
	ev.data.ptr = nc;
	if (epoll_ctl(nc->mgr->epoll_fd, op, nc->sock, &ev) < 0) {
		LOG(LL_ERROR,( "epoll_ctl, %d, error %p, sock:%d", op, nc, nc->sock));
	}
	else {
		DBG(("epoll_ctl succ, nc:%p, fd:%d, events:%d, op:%d", nc, nc->sock, ev.events, op));
	}
}
#endif
...
void mg_mgr_init_opt(struct mg_mgr *m, void *user_data,
...
#ifdef __unix__
  m->ctl_con[0] = (struct mg_connection *) MG_CALLOC(1, sizeof(struct mg_connection));
  m->ctl_con[0]->mgr = m;
  m->ctl_con[0]->sock = INVALID_SOCKET;
  m->ctl_con[1] = (struct mg_connection *) MG_CALLOC(1, sizeof(struct mg_connection));
  m->ctl_con[1]->sock = INVALID_SOCKET;
  m->ctl_con[1]->mgr = m;
  ...
  } while (m->epoll_fd  < 0 && EINTR == errno);
  if (m->epoll_fd < 0)  {
	  LOG(LL_ERROR, ("epoll_create1 error"));
  }
...
#ifdef __unix__
  mg_remove_from_epoll(m->ctl_con[0]);
  mg_remove_from_epoll(m->ctl_con[1]);
  mg_destroy_conn(m->ctl_con[0], 0);
  mg_destroy_conn(m->ctl_con[1], 0);
  m->ctl_con[0] = m->ctl_con[1] = NULL;
#endif
#if MG_ENABLE_BROADCAST
...
#if MG_ENABLE_BROADCAST
#ifdef __unix__
  mgr->ctl_con[1]->sock = mgr->ctl[1];
  if (mgr->ctl_con[1]->epoll_events == 0 && mgr->ctl_con[1]->sock != INVALID_SOCKET)
  {
	  mgr->ctl_con[1]->epoll_events |= EPOLLIN;
	  mg_add_to_epoll(mgr->ctl_con[1], EPOLL_CTL_ADD);
  }
...
#ifdef __unix__
		  if (!(nc->epoll_events & EPOLLIN))
		  {
			  int op = (nc->epoll_events == 0) ? EPOLL_CTL_ADD : EPOLL_CTL_MOD;
			  nc->epoll_events |= EPOLLIN;
			  mg_add_to_epoll(nc, op);
		  }
#else
...
#ifdef __unix__
		  if (!(nc->epoll_events & EPOLLOUT) || !(nc->epoll_events & EPOLLERR))
		  {
			  int op = (nc->epoll_events == 0) ? EPOLL_CTL_ADD : EPOLL_CTL_MOD;
			  nc->epoll_events |= EPOLLOUT;
			  nc->epoll_events |= EPOLLERR;
			  mg_add_to_epoll(nc, op);
		  }
#else
...
#ifdef __unix__
	  if (nc->epoll_events == 0) {
		  ev.events = nc->epoll_events;
		  ev.data.ptr = NULL;
		  if (epoll_ctl(mgr->epoll_fd, EPOLL_CTL_DEL, nc->sock, &ev) < 0) {
			  LOG(LL_ERROR, ("epoll_ctl, error, %p, sock:%d", nc, nc->sock));
		  }
	  }
#endif
...
#ifdef __unix__
  num_ev = epoll_wait(mgr->epoll_fd, events, EPOLL_MAX_EVENTS, timeout_ms);
#else
...
#ifdef __unix__
  for (event_index = 0; event_index <num_ev; ++event_index) {
	  int fd_flags = 0;
	  nc = events[event_index].data.ptr;
	  if (nc->sock != INVALID_SOCKET) {
		  if (events[event_index].events & EPOLLIN) {
			  if (nc == mgr->ctl_con[1]) {
				  //nc->epoll_events &= ~EPOLLIN;
		  ...
		  if (events[event_index].events & EPOLLOUT) {
			  fd_flags |= _MG_F_FD_CAN_WRITE;
			  //nc->epoll_events &= ~EPOLLOUT;
		  }
		  if (events[event_index].events & EPOLLERR) {
			  fd_flags |= _MG_F_FD_ERROR;
			  //nc->epoll_events &= ~EPOLLERR;
		  }
#if MG_LWIP
