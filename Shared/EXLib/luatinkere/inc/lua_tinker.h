...
namespace lua_tinker
{
	...
	static inline void scope_inner(lua_State* L, const char* scope_global_name, const char* name, const char* global_name);
	//namespace
	static inline void namespace_add(lua_State* L, const char* namespace_name);
	template<typename T>
		...
		struct member_functor : public functor_base
		{
			...
			virtual int apply(lua_State* L) override
			{
				...
				TRY_LUA_TINKER_INVOKE()
				{
					...
					CT* pCTPtr = _read_classptr_from_index1<CT, bConst>(L);
					if (pCTPtr)
					{
						_invoke_function<RVal>(L, m_func, pCTPtr);
					}
					return 1;
				}
			...
			static int invoke(lua_State *L)
			{
				...
				TRY_LUA_TINKER_INVOKE()
				{
					...
					if (pCTPtr)
					{
						_invoke<RVal>(L, upvalue_<FuncType>(L), pCTPtr);
					}
					return 1;
				}
				CATCH_LUA_TINKER_INVOKE()
			...
			static int invoke_function(lua_State *L)
			{
				...
				TRY_LUA_TINKER_INVOKE()
				{
					...
					if (pCTPtr)
					{
						_invoke_function<RVal>(L, upvalue_<FuncWarpType*>(L)->m_pfunc, pCTPtr);
					}
					return 1;
				}
				CATCH_LUA_TINKER_INVOKE()
